...
public class DifferentSearchTree {
    ...
    public int calculate04(int n) {
        ...
        for (int i = 2; i < n + 1; i++) {
            for (int j = 1; j < i + 1; j++) {
                dp[i] += dp[j - 1] * dp[i - j]; // 数学公式进行累加
            }
        }
        return dp[n];
    }

    // 注意dp[2] = 2; // 2有两个对应的搜索二叉树
    public int calculate05(int n) {
        if (n < 0) {
            return -1;
        }
        int[] dp = new int[n + 1];
    ...
    public static void main(String[] args) {
        ...
        int n = 5;
        int value = differentSearchTree.calculate08(5);

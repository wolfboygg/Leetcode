...
public class ArrayFindIndex {
    ...
    public int[] find07(int[] arr, int target) {
        if (arr == null || arr.length == 0) {
            return null;
        }
        int leftIndex = realFind07(arr, target, true);
        int rightIndex = realFind07(arr, target, false) - 1;
        return new int[] {leftIndex, rightIndex};
    }

    public int realFind07(int[] arr, int target, boolean isLow) {
        int low = 0;
        int height = arr.length - 1;
        int targetIndex = 0;
        while(low <= height) { // 这里必须使用相等处理
            int middle = low + (height - low) / 2;
            System.out.println("low:" + low + ",height:" + height + "middle:" + middle);
            if (arr[middle] > target || (isLow && arr[middle] >= target)) {
                height = middle - 1;
                targetIndex = middle;
            } else {
    ...
    public int[] find08(int[] arr, int target) {
        if (arr == null || arr.length == 0) {
            return new int[]{};
        }
    ...
    public int[] find10(int[] arr, int target) {
        if (arr == null || arr.length == 0) {
            return null;
        }
        int leftIndex = realFind10(arr, target, true);
        int rightIndex = realFind10(arr, target, false) - 1;
        return new int[] { leftIndex, rightIndex};
    }

    public int realFind10(int[] arr, int target, boolean isLow) {
        int left = 0;
        int right = arr.length - 1;
        int targetIndex = -1;
        while(left <= right) {
            int middle = left + (right - left) / 2;
            if (arr[middle] > target || (arr[middle] >= target && isLow)) { // 这里要看大的
                right = middle - 1;
                targetIndex = middle;
            } else {
                left = middle + 1;

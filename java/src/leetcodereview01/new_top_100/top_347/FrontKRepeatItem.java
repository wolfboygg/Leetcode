...
public class FrontKRepeatItem {
    ...
    // 使用小头堆进行求解
    public List<Integer> findItem09(int[] arr, int k) {
        if (arr == null || arr.length == 0) {
            return null;
        }
        Map<Integer, Integer> map = new HashMap<>();
        ...
        while(iterator.hasNext()) {
            ...
            if (queue.size() < k) {
                ...
                if (map.get(queue.peek()) < map.get(next)) {
                    queue.poll();
                    queue.offer(next);
                }
            }
    ...
    public List<Integer> findItem11(int[] arr, int k) {
        if (arr == null || arr.length == 0) {
            return null;
        }
        Map<Integer, Integer> map = new HashMap<>();
    ...
    public List<Integer> findItem12(int[] arr, int k) {
        if (arr == null || arr.length == 0) {
            return null;
        }
        Map<Integer, Integer> map = new HashMap<>();
        ...
        while(iterator.hasNext()) {
            ...
            if (queue.size() < k) {
                ...
                if (map.get(next) > map.get(queue.peek())) {
                    queue.poll();
                    queue.offer(next);
                }
            }
    ...
    public static void main(String[] args) {
        ...
        int k = 2;
        List<Integer> result = repeatItem.findItem12(arr, k);
